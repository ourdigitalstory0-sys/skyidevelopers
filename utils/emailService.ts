...
/**
 * Dispatches enquiry leads to propsmartrealty@gmail.com using FormSubmit API
 * with optional Nodemailer Gmail SMTP fallback.
 */
export async function sendLeadEmailNotification(params: SendLeadEmailParams): Promise<boolean> {
  const nowIST = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' });

  // Log lead details internally
  logger.log(`[Lead Dispatch Engine] Processing Lead ${params.leadId}`, params);

  let formSubmitSuccess = false;

  // 1. Primary Dispatch: FormSubmit.co AJAX API (zero-config, high-reliability)
  try {
    const formSubmitPayload = {
      _subject: `🔥 NEW SKYi LEAD [${params.name}] — ${params.project || 'NA Bungalow Plots Pune'} (+91 ${params.phone})`,
      _template: 'table',
      _captcha: 'false',
      'Lead Reference ID': params.leadId,
      'Customer Name': params.name,
      'Mobile Phone Number': `+91 ${params.phone}`,
      'Email Address': params.email || 'Not Provided',
      'Interested Project / Location': params.project || 'General Inquiry / NA Plots',
      'Site Visit Date & Time': params.visitDate ? `${params.visitDate} at ${params.visitTime || '10:00 AM'}` : 'Not Requested',
      'Cab Pickup Required': params.pickupNeeded ? 'YES (Free Pickup Needed)' : 'NO',
      'Customer Requirements / Message': params.message || 'None',
      'Traffic / Source Channel': params.source || 'Website Form',
      'Submission Timestamp (IST)': nowIST,
    };
    ...
    if (response.ok) {
      ...
      logger.log(`[FormSubmit Success] Lead ${params.leadId} delivered to ${TARGET_EMAIL}`, data);
      formSubmitSuccess = true;
    } else {
      logger.error(`[FormSubmit HTTP Error] Status ${response.status}`, await response.text());
    }
  } catch (formSubmitErr) {
    logger.error(`[FormSubmit Network Error] Failed for lead ${params.leadId}`, formSubmitErr);
  }

  // 2. Secondary Dispatch: Gmail SMTP via Nodemailer (if GMAIL_APP_PASSWORD configured)
  ...
  if (gmailPass) {
    try {
      ...
      logger.log(`[SMTP Success] Lead Email Sent to ${TARGET_EMAIL}`, info.messageId);
    } catch (smtpError) {
      logger.error(`[SMTP Error] Failed for lead ${params.leadId}`, smtpError);
    }
  }
